...
function HomePage({ user, cart, showAuth, showToast, onTabChange, banners, setBanners, addToCart }) {
  ...
  useEffect(() => {
    ...
    sbGetCategories().then(rows => {
      if (rows && rows.length) {
        setDbCategories(rows.filter(r => r.active !== false).sort((a,b) => (a.sort_order||0)-(b.sort_order||0)));
      }
    }).catch(()=>{});
  }, []);
  ...
  return (
    <div style={{ background:"#f5f7fa",minHeight:"100vh",paddingBottom:70,fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display:"flex",gap:8,overflowX:"auto",paddingBottom:10,scrollbarWidth:"none",msOverflowStyle:"none",WebkitOverflowScrolling:"touch" }}>
          {cats.map(cat => (
            <button key={cat.name} onClick={() => setActiveCat(cat.name)} style={{
              ...
              background: activeCat===cat.name ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.1)",
